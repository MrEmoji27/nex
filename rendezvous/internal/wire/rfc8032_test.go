package wire

import (
	"crypto/ed25519"
	"encoding/hex"
	"testing"
)

// rfc8032Section71 holds the official RFC 8032 §7.1 Ed25519 test vectors,
// transcribed verbatim from the RFC (TEST 1, TEST 2, TEST 3, TEST 1024 and
// TEST SHA(abc)).
//
// Contract §1.2 makes this test mandatory. Passing only against the other Nex
// implementation is explicitly insufficient: a prior audit caught a
// self-consistent "Noise" implementation that was not spec-conformant, and the
// only thing that caught it was foreign vectors. These values are published
// constants — they must never be regenerated from this codebase's own crypto.
var rfc8032Section71 = []struct {
	name   string
	secret string // 32-byte Ed25519 seed
	public string // 32-byte public key
	msg    string // message, hex
	sig    string // 64-byte signature
}{
	{
		name:   "TEST 1",
		secret: "9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60",
		public: "d75a980182b10ab7d54bfed3c964073a0ee172f3daa62325af021a68f707511a",
		msg:    "",
		sig:    "e5564300c360ac729086e2cc806e828a84877f1eb8e5d974d873e065224901555fb8821590a33bacc61e39701cf9b46bd25bf5f0595bbe24655141438e7a100b",
	},
	{
		name:   "TEST 2",
		secret: "4ccd089b28ff96da9db6c346ec114e0f5b8a319f35aba624da8cf6ed4fb8a6fb",
		public: "3d4017c3e843895a92b70aa74d1b7ebc9c982ccf2ec4968cc0cd55f12af4660c",
		msg:    "72",
		sig:    "92a009a9f0d4cab8720e820b5f642540a2b27b5416503f8fb3762223ebdb69da085ac1e43e15996e458f3613d0f11d8c387b2eaeb4302aeeb00d291612bb0c00",
	},
	{
		name:   "TEST 3",
		secret: "c5aa8df43f9f837bedb7442f31dcb7b166d38535076f094b85ce3a2e0b4458f7",
		public: "fc51cd8e6218a1a38da47ed00230f0580816ed13ba3303ac5deb911548908025",
		msg:    "af82",
		sig:    "6291d657deec24024827e69c3abe01a30ce548a284743a445e3680d7db5ac3ac18ff9b538d16f290ae67f760984dc6594a7c15e9716ed28dc027beceea1ec40a",
	},
	{
		name:   "TEST 1024",
		secret: "f5e5767cf153319517630f226876b86c8160cc583bc013744c6bf255f5cc0ee5",
		public: "278117fc144c72340f67d0f2316e8386ceffbf2b2428c9c51fef7c597f1d426e",
		msg:    "",
		sig:    "0aab4c900501b3e24d7cdf4663326a3a87df5e4843b2cbdb67cbf6e460fec350aa5371b1508f9f4528ecea23c436d94b5e8fcd4f681e30a6ac00a9704a188a03",
	},
	{
		name:   "TEST SHA(abc)",
		secret: "833fe62409237b9d62ec77587520911e9a759cec1d19755b7da901b96dca3d42",
		public: "ec172b93ad5e563bf4932c70e1245034c35467ef2efd4d64ebf819683467e2bf",
		msg:    "ddaf35a193617abacc417349ae20413112e6fa4e89a97ea20a9eeee64b55d39a2192992a274fc1a836ba3c23a3feebbd454d4423643ce80e2a9ac94fa54ca49f",
		sig:    "dc2a4459e7369633a52b1bf277839a00201009a3efbf3ecb69bea2186c26b58909351fc9ac90b3ecfdfbc7c66431e0303dca179c138ac17ad9bef1177331a704",
	},
}

func TestRFC8032Section71Vectors(t *testing.T) {
	for _, v := range rfc8032Section71 {
		t.Run(v.name, func(t *testing.T) {
			seed, err := hex.DecodeString(v.secret)
			if err != nil || len(seed) != ed25519.SeedSize {
				t.Fatalf("bad secret key vector: %v", err)
			}
			wantPub, err := hex.DecodeString(v.public)
			if err != nil {
				t.Fatalf("bad public key vector: %v", err)
			}
			msg, err := hex.DecodeString(v.msg)
			if err != nil {
				t.Fatalf("bad message vector: %v", err)
			}
			wantSig, err := hex.DecodeString(v.sig)
			if err != nil {
				t.Fatalf("bad signature vector: %v", err)
			}

			priv := ed25519.NewKeyFromSeed(seed)
			gotPub := priv.Public().(ed25519.PublicKey)
			if hex.EncodeToString(gotPub) != v.public {
				t.Fatalf("public key mismatch:\n got %x\nwant %s", gotPub, v.public)
			}

			gotSig := ed25519.Sign(priv, msg)
			if hex.EncodeToString(gotSig) != v.sig {
				t.Fatalf("signature mismatch:\n got %x\nwant %s", gotSig, v.sig)
			}

			if !ed25519.Verify(wantPub, msg, wantSig) {
				t.Fatal("published signature failed to verify against published public key")
			}

			// The service only ever verifies; exercise the hex-wrapped path too.
			if !Verify(v.public, msg, v.sig) {
				t.Fatal("wire.Verify rejected a published RFC 8032 vector")
			}
		})
	}
}

// TestRFC8032VectorsRejectTampering guards the negative direction: the same
// vectors must fail once anything moves.
func TestRFC8032VectorsRejectTampering(t *testing.T) {
	for _, v := range rfc8032Section71 {
		t.Run(v.name, func(t *testing.T) {
			msg, _ := hex.DecodeString(v.msg)

			// Flip one bit of the signature.
			bad := []byte(v.sig)
			if bad[0] == '0' {
				bad[0] = '1'
			} else {
				bad[0] = '0'
			}
			if Verify(v.public, msg, string(bad)) {
				t.Fatal("mutated signature verified")
			}

			// Append a byte to the message.
			if Verify(v.public, append(append([]byte{}, msg...), 0x00), v.sig) {
				t.Fatal("signature verified over an extended message")
			}

			// Verify against a different vector's key.
			other := rfc8032Section71[(len(rfc8032Section71)-1)%len(rfc8032Section71)].public
			if other != v.public && Verify(other, msg, v.sig) {
				t.Fatal("signature verified under the wrong public key")
			}
		})
	}
}

// TestVerifyRejectsMalformedEncodings pins the field-shape rules of §1.3/§4:
// signPub is lowercase hex(64), sig is lowercase hex(128). Uppercase hex is a
// different wire encoding and must be rejected, not silently accepted.
func TestVerifyRejectsMalformedEncodings(t *testing.T) {
	v := rfc8032Section71[1]
	msg, _ := hex.DecodeString(v.msg)

	cases := map[string]struct{ pub, sig string }{
		"uppercase pub": {upper(v.public), v.sig},
		"uppercase sig": {v.public, upper(v.sig)},
		"short pub":     {v.public[:62], v.sig},
		"short sig":     {v.public, v.sig[:126]},
		"empty pub":     {"", v.sig},
		"empty sig":     {v.public, ""},
		"non-hex pub":   {"zz" + v.public[2:], v.sig},
		"pub with 0x":   {"0x" + v.public[2:], v.sig},
		"sig too long":  {v.public, v.sig + "00"},
		"pub too long":  {v.public + "00", v.sig},
	}
	for name, c := range cases {
		if Verify(c.pub, msg, c.sig) {
			t.Errorf("%s: Verify accepted a malformed encoding", name)
		}
	}
}

func upper(s string) string {
	b := []byte(s)
	for i := range b {
		if b[i] >= 'a' && b[i] <= 'f' {
			b[i] -= 32
		}
	}
	return string(b)
}
