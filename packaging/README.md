...
2. Download `Nex-setup-3.0.0-alpha.6.exe`
3. Verify SHA256: `57d6ca51f7dfa152b5e420c70f86690aaad8ea1c3f27cc8372b8bc138a36d487`
   ```powershell
   Get-FileHash Nex-setup-3.0.0-alpha.6.exe -Algorithm SHA256
   ```
...
> **Important:** As of `v3.0.0-alpha.6`, **no macOS or Linux binaries are published yet**. The script will detect this and exit with instructions to build from source (see below). When binaries are published, the same command will download and install them.
...
1. Tag the release:
   ```bash
   git tag -a v3.0.0-alpha.6 -m "Nex 3.0.0-alpha.6"
   git push origin v3.0.0-alpha.6
   ```
...
2. Build the Windows installer:
   ...
   ./build-installer.sh
   ```
   Outputs `../dist/Nex-setup-3.0.0-alpha.6.exe`
